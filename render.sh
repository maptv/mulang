...
cat "${1%.*}.qmd"  | gsed '/^```{/,/^```$/ { # set a range and say what should happen in that range below
    ...
    s/^```{\(.*\)}#| tags: \[\(.*\)\]/{{< embed _\1\.ipynb#\2 >}}/g; # replace code chunk and comment syntax with shortcode syntax
    /{{< embed .*\.ipynb#.* >}}/!d # delete anything that is not an embed shortcode with a tag
}' | sed "s/{{< embed \(.*\)\.ipynb#\(.*\) >}}/{{< embed ${1%.*}_jupytext\1\.ipynb#\2 >}}/g"> "${1%.*}_embed.qmd"

# Here is the above command in a single line.
...
# cat "${1%.*}.qmd" | gsed '/^```{/,/^```$/ { /^```{.*}$\|#| tags:/!d; N;s/\n//g; s/^```{\(.*\)}#| tags: \[\(.*\)\]/{{< embed _\1\.ipynb#\2 >}}/g; /{{< embed .*\.ipynb#.* >}}/!d }' > "${1%.*}_embed.qmd"
